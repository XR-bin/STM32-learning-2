...
void TIM5_IRQHandler(void)
{
  ...
  //边沿触发中断
  if(TIM_GetITStatus(TIM5,TIM_FLAG_CC1) != RESET)
  {
		//清中断标志位
...
void TIM1_BRK_TIM9_IRQHandler(void)
{
	...
	if(TIM_GetITStatus(TIM9,TIM_FLAG_CC2) != RESET)
	{
		//清除捕通道IC2的获标志位
		TIM_ClearITPendingBit(TIM9,TIM_FLAG_CC2);
		//紧急事件
		//CCR2
		data_2 = TIM_GetCapture2(TIM9);	
	}
	
	if(TIM_GetITStatus(TIM9,TIM_FLAG_CC1) != RESET)
	{
		//清除通道1的捕获标志位
		TIM_ClearITPendingBit(TIM9,TIM_FLAG_CC1);
		//清除通道1重复捕获标志位
		TIM_ClearITPendingBit(TIM9,TIM_FLAG_CC1OF);
		//紧急事件
		//记录值CCR1
		data_1 = TIM_GetCapture1(TIM9);
		//计算周期和占空比
