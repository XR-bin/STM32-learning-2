...
void ADC1_Init2_In5(void)
{
  ...
  ADC_RegularChannelConfig(ADC1, ADC_Channel_5, 1,ADC_SampleTime_480Cycles);  
	...
	/*中断配置*/
	ADC_ITConfig(ADC1,ADC_IT_EOC,ENABLE);   //打开ADC1的EOC中断(ADC->CR1寄存器的第5位)
...
void ADC2_Init_In4In5(void)
{
  ...
  /*GPIOx端口配置*/
  //PA4
  GPIO_InitStruc.GPIO_Pin   = GPIO_Pin_4|GPIO_Pin_5;       //选择PA4和PA5
  GPIO_InitStruc.GPIO_Mode  = GPIO_Mode_AN;                //模拟输入
  GPIO_InitStruc.GPIO_PuPd  = GPIO_PuPd_NOPULL;            //无上下拉
  GPIO_Init(GPIOA,&GPIO_InitStruc);                        //根据设定参数初始化PA4和PA5
  ...
  ADC_InitStruct.ADC_Resolution = ADC_Resolution_12b;                      //12位模式(ADC->CR1寄存器的第24、25位)
  ADC_InitStruct.ADC_ScanConvMode = ENABLE;                                //扫描模式(ADC->CR1寄存器的第8位)
  ADC_InitStruct.ADC_ContinuousConvMode = ENABLE;                          //连续转换(ADC->CR2寄存器的第1位)
  ADC_InitStruct.ADC_ExternalTrigConvEdge = ADC_ExternalTrigConvEdge_None; //禁止触发检测，使用软件触发(ADC->CR2寄存器的第20-21位/第28-29位)
  ADC_InitStruct.ADC_DataAlign = ADC_DataAlign_Right;                      //右对齐	(ADC->CR2寄存器的第11位)
  ADC_InitStruct.ADC_NbrOfConversion = 2;                                  //2个转换在规则序列中 也就是只转换规则序列1(ADC->SQR1寄存器的第20-23位)
  ADC_Init(ADC2, &ADC_InitStruct);                                         //ADCx配置初始化
	
	//ADC规则通道序列偏序及采样时间配置函数
  //ADC2的第一个转换时通道4，采样时间480个周期        (采样：ADCx->SMPRx)
  ADC_RegularChannelConfig(ADC2, ADC_Channel_4, 1,ADC_SampleTime_480Cycles);  
	//ADC2的第二个转换时通道5，采样时间480个周期        (采样：ADCx->SMPRx)
...
ADC2_DATA ADC2_In4In5_Data(void)
{
  ...
  //等待转换完成   第一次
	while(!ADC_GetFlagStatus(ADC2, ADC_FLAG_EOC )){};//等待转换结束(ADC2->SR寄存器的第1位)
  //读取转换后的数据(ADC1->DR)
  data.adc2_LDR = ADC_GetConversionValue(ADC2); 
  //等待转换完成   第二次		
	while(!ADC_GetFlagStatus(ADC2, ADC_FLAG_EOC )){};//等待转换结束(ADC2->SR寄存器的第1位)
  //读取转换后的数据(ADC1->DR)
  data.adc2_R45 = ADC_GetConversionValue(ADC2); 
