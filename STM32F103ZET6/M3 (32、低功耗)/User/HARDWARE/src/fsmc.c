...
*************************************************************************/
	ReadTiming.FSMC_AddressSetupTime = 0X01;	     //地址设置阶段的持续时间：2个HCLK 1/36M=27ns	               (FSMC->BTR寄存器的第0-3位)
	ReadTiming.FSMC_AddressHoldTime = 0x00;	       //地址保持阶段的持续时间:Reserved(模式A未用到)               (FSMC->BTR寄存器的第4-7位)
  ReadTiming.FSMC_DataSetupTime = 0x0f;			     //数据阶段的持续时间:16个HCLK=6*8=48ns                       (FSMC->BTR寄存器的第8-15位)   数据保存时间为16个HCLK,因为液晶驱动IC的读数据的时候，速度不能太快，尤其对1289这个IC。
  ...
  ReadTiming.FSMC_AccessMode = FSMC_AccessMode_A;//访问模式：访问模式 A                                        (FSMC->BTR寄存器的第28-29位) 
	...
	WriteTiming.FSMC_AddressSetupTime =0x0;	        //地址设置阶段的持续时间：1个HCLK 	                          (FSMC->BWTR寄存器的第0-3位)
  WriteTiming.FSMC_AddressHoldTime = 0x0;	        //地址保持阶段的持续时间:Reserved(模式A未用到)                (FSMC->BWTR寄存器的第4-7位)	
  WriteTiming.FSMC_DataSetupTime = 0x03;		      //数据阶段的持续时间:4个HCLK=6*4=24ns                         (FSMC->BWTR寄存器的第8-15位)
