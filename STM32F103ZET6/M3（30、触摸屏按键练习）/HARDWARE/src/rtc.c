...
void MyRTC_Init(RTC_DATES rtc)
{
  ...
  //使能电源控制器时钟
	...
	if(BKP_ReadBackupRegister(BKP_DR1)!=0xff)//用于避免复位后重新设时间值(没电池时只对复位有用，断电后该位会清零，依然会重新设时间值)
	{
		//复位备份区域
		BKP_DeInit();
		
		/*时钟源选择*/
		RCC_RTCCLKConfig(RCC_RTCCLKSource_LSE);		            //选择外部低速时钟作为时钟源(RCC->BDCR寄存器的第8-9位)
		RCC_LSEConfig(RCC_LSE_ON);                            //开启外部低速震荡器(LSE) (RCC->BDCR寄存器的第0位)
		while(RCC_GetFlagStatus(RCC_FLAG_LSERDY) == RESET);   //外部低速震荡器就绪 (RCC->BDCR寄存器的第1位)
    ...
    RTC_Set(rtc);
		...
		BKP_WriteBackupRegister(BKP_DR1, 0xff);	//向指定的后备寄存器中写入用户程序数据(BKP_DRx寄存器)
	}
}
